...
const rawPages = import.meta.glob('../pages/**/*.tsx', { query: '?raw', eager: true });
...
for (const path in rawPages) {
  ...
  const cleanContent = typeof content === 'string' 
    ? content.replace(/className="[^"]*"/g, '').replace(/<svg[\s\S]*?<\/svg>/g, '') 
    : '';
  WEBSITE_KNOWLEDGE += `\n\n--- Page: ${path} ---\n${cleanContent}`;
}
...
export default function AIChatBot() {
  ...
  return (
    <>
      <div ref={containerRef} className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-40 flex flex-col items-start">
        <AnimatePresence>
          {isOpen && (
            <motion.div
...
<div className="bg-white p-0.5 rounded-full overflow-hidden border border-amber-200">
  <svg viewBox="0 0 100 100" className="w-7 h-7" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M20 100 A30 30 0 0 1 80 100 Z" fill="#ffffff" stroke="#e2e8f0" strokeWidth="2" />
    <path d="M 25 45 Q 50 20 75 45 L 75 35 A 25 25 0 0 0 25 35 Z" fill="#3f3f46" />
    <circle cx="35" cy="45" r="4" fill="#3f3f46" />
    <circle cx="65" cy="45" r="4" fill="#3f3f46" />
    <path d="M 45 60 Q 50 65 55 60" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" />
    <path d="M 30 70 Q 50 90 70 70" fill="none" stroke="#475569" strokeWidth="3" strokeLinecap="round" />
    <circle cx="70" cy="70" r="4" fill="#cbd5e1" />
  </svg>
</div>
                  <div>
                    <h3 className="font-semibold text-sm">Ayurveda Assistant</h3>
                    <p className="text-amber-100 text-xs">Sattvic Ayurveda & Panchakarma</p>
                  </div>
                </div>
                <button 
                      ...
                      >
                        {msg.role === 'user' ? (
                          ...
                          <div className="prose prose-sm prose-amber max-w-none prose-p:leading-snug prose-p:mb-2 last:prose-p:mb-0">
                            <Markdown 
                              components={{
                                a: ({node, ...props}) => <a {...props} target="_blank" rel="noopener noreferrer" className="font-semibold text-amber-700 hover:text-amber-800 underline underline-offset-2" />
                              }}
                            >
                              {msg.text}
                            </Markdown>
        ...
        <div className="relative flex items-center">
          ...
          <AnimatePresence>
            {!isOpen && showGreeting && (
              ...
              >
                Hi, I'm <span className="font-semibold text-amber-700">AI-Vaidya!</span><br/>
                <span className="text-stone-500 text-xs">How may I help you?</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
