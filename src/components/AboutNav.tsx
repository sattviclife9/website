...
const navItems = [
  { name: 'Our Story & Centre', path: '/about-sattvic-ayurveda' },
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 md:gap-4 pb-1 lg:pb-0">
          {navItems.map((item) => {
            const isActive = location.pathname === item.path || 
              (item.path === '/about-sattvic-ayurveda' && location.pathname === '/about-sattvic-ayurveda/centre');
            return (
              <Link 
                key={item.path} 
                to={item.path}
                className={`relative whitespace-nowrap px-5 py-2 rounded-full text-[10px] md:text-[11px] uppercase tracking-[0.15em] font-bold transition-all duration-300 ${
                  isActive 
